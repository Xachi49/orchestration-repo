...
try {
  execSync(
    "npx vitest run --fileParallelism=false src/infrastructure/postgres/postgres.durability.test.ts src/infrastructure/postgres/postgres.integration.test.ts src/infrastructure/postgres/postgres.resource-ledger.test.ts src/infrastructure/postgres/postgres.acceptance.test.ts",
    {
      stdio: "inherit",
      env: process.env,
    },
