/**
 * Planning model that proposes exactly one bounded SEND_RECOVERY_EMAIL step.
...
export const RECOVERY_EMAIL_POSTCONDITION =
  "Bounded recovery outreach attempt recorded";

export type RecoveryEmailPlanBinding = {
 ...
 */
export function createRecoveryEmailPlanningModel(
  binding: RecoveryEmailPlanBinding,
): FakePlanningModel {
  return new RecoveryEmailPlanningModel(binding);
}

class RecoveryEmailPlanningModel extends FakePlanningModel {
  constructor(private readonly binding: RecoveryEmailPlanBinding) {
    super();
  }
  ...
  override async proposePlan(input: {
    ...
    if (
      ...
      throw new Error(
        "Recovery email planning binding is incomplete — set case/lead/template before plan()",
      );
    }
    const steps: PlanProposal["steps"] = [
      {
        ...
        description: "Send one bounded recovery email to the canonical lead",
        targetIds: [
        ...
        dependsOn: [],
        preconditions: [
          "Recovery case is active and contact policy permits EMAIL",
        ],
        expectedPostconditions: [RECOVERY_EMAIL_POSTCONDITION],
        resourceEstimate: {
          durationMs: 20_000,
          tokenEstimate: 200,
    ...
    return {
      value: parsePlanProposal({
        ...
        workstreams: [
          {
            ...
            name: "Bounded recovery email outreach",
        ...
        conciseRationale:
          "One bounded email step whose recipient resolves from the canonical lead.",
      }),
